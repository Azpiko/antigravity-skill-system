...
function updateTokenUsage() {
  ...
  if (args.length < 3) {
    console.error('Usage: tsx track-tokens.ts "Description" <prompt_est> <completion_est>');
    process.exit(1);
  }

  const [description, promptEst, completionEst] = args;
  const now = new Date();
  ...
  for (const line of lines) {
    // Regex plus permissive pour capturer les colonnes du tableau
    const match = line.match(/\|\s*\d{2}\/\d{2}\/\d{4}\s*\|\s*(\d+)\s*\|\s*[^|]+\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*(\d+)\s*\|/);
    if (match) {
  ...
  const newRow = `| ${timestamp} | ${newTurnId} | ${description} | ${promptEst} | ${completionEst} | ${newCumulativeTotal} |`;
  ...
  content = lines.join('\n');
  content = content.replace(/- \*\*Total Jetons Estimés\*\* : ~(\d+)/, `- **Total Jetons Estimés** : ~${newCumulativeTotal}`);
  content = content.replace(/- \*\*Dernière mise à jour\*\* : (?:[\d: ]+)?\d{2}\/\d{2}\/\d{4}/, `- **Dernière mise à jour** : ${timestamp}`);

  fs.writeFileSync(REPORT_FILE, content);
  console.log(`✅ Suivi mis à jour : Turn ${newTurnId} (+${turnTotal} tokens) à ${timeStr}`);
}
